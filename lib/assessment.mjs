...
export function prepareQuestion(question, index) {
  ...
  return { id, domain: question.domain, type: question.type, prompt: question.prompt, options, answer: target, uitleg: question.uitleg };
}

// De assessment eindigt altijd met Engels (leesbegrip) als laatste blok en
// Werkhouding & Communicatie als één-na-laatste blok. Andere domeinen komen ervoor.
const TAIL_ORDER = ['Werkhouding & Communicatie', 'Engels'];
function withTailLast(relevant) {
  const head = relevant.filter((d) => !TAIL_ORDER.includes(d));
  const tail = TAIL_ORDER.filter((d) => relevant.includes(d));
  return [...head, ...tail];
}
...
export function selectQuestionsForRole(roleId, approved = [], { domains: override, minPerDomain = 5, weightThreshold = 0.07 } = {}) {
  ...
  const ordered = withTailLast(relevant);
  const seed = testQuestions.map((q, i) => ({ ...q, id: q.id ?? `seed-${i}` }));
  const pool = [...seed, ...approved];
  const chosen = [];
  for (const domain of ordered) chosen.push(...pool.filter((q) => q.domain === domain).slice(0, minPerDomain));
  return chosen.map((q, i) => prepareQuestion(q, i));
}

export function toClientQuestion(q) {
  // answer én uitleg eraf: de kandidaat mag het juiste antwoord/de uitleg nooit zien.
  const { answer, uitleg, ...rest } = q;
  return rest;
}
...
export function gradeDetails(answers, questions) {
  ...
  return questions.map((q) => {
    ...
    return { questionId: q.id, domain: q.domain, prompt: q.prompt, options: q.options, chosenIndex, correctIndex: q.answer, correct: chosenIndex === q.answer, uitleg: q.uitleg };
  });
}

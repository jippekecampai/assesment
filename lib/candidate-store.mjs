...
// Azure Table String-property max = 64 KB (~32K UTF-16 tekens). Grote rollen leveren
// veel vragen op; we splitsen de JSON daarom in stukken (data0..dataN) ruim onder
// de limiet. Oud formaat (één `data`-veld) blijft leesbaar.
export const DATA_CHUNK = 28000;
export function packData(obj) {
  const json = JSON.stringify(obj);
  const out = {};
  let n = 0;
  for (let i = 0; i < json.length; i += DATA_CHUNK) { out[`data${n}`] = json.slice(i, i + DATA_CHUNK); n += 1; }
  out.dataChunks = n;
  return out;
}
export function unpackData(entity) {
  if (entity.dataChunks != null) {
    let s = '';
    for (let i = 0; i < entity.dataChunks; i += 1) s += entity[`data${i}`] || '';
    return s;
  }
  return entity.data; // oud formaat (één veld)
}
...
function azureStore(connectionString, tableName) {
  ...
  return {
    async createCandidate(input) {
      ...
      await c.createEntity({ partitionKey: 'candidate', rowKey: candidate.id, code: candidate.code, ...packData(candidate) });
      return candidate;
    },
    async getByCode(code) {
      ...
      for await (const e of it) return JSON.parse(unpackData(e));
      return null;
    },
    async listCandidates() {
      const c = await client(); const out = [];
      for await (const e of c.listEntities({ queryOptions: { filter: `PartitionKey eq 'candidate'` } })) out.push(JSON.parse(unpackData(e)));
    ...
    async updateCandidate(id, patch) {
      ...
      const candidate = { ...JSON.parse(unpackData(e)), ...patch };
      await c.updateEntity({ partitionKey: 'candidate', rowKey: id, code: candidate.code, ...packData(candidate) }, 'Replace');
      return candidate;
    },
    async saveResult(candidateId, result) {
      const c = await client();
      await c.upsertEntity({ partitionKey: 'result', rowKey: candidateId, ...packData(result) }, 'Replace');
    },
    async getResult(candidateId) {
      const c = await client();
      try { return JSON.parse(unpackData(await c.getEntity('result', candidateId))); }
      catch (e) { if (e.statusCode === 404) return null; throw e; }
    },
    async deleteCandidate(id) {
      const c = await client();
      await c.deleteEntity('candidate', id).catch((e) => { if (e.statusCode !== 404) throw e; });
      await c.deleteEntity('result', id).catch((e) => { if (e.statusCode !== 404) throw e; });
    },
  };
}
